...
def describeAlarm():
	...
	# Convert from UTC
	utcschedule = argoneonrtc.describeSchedule([-1], [weekday], [date], [hour], [minute])
	weekday, date, hour, minute = argonrtc.convertAlarmTimezone(weekday, date, hour, minute, False)

	return argoneonrtc.describeSchedule([-1], [weekday], [date], [hour], [minute]) + " Local (RTC Schedule: "+utcschedule+" UTC)"
...
def setRTCAlarm(enableflag, weekday, date, hour, minute):
	...
	weekday, date, hour, minute = argonrtc.getRTCAlarm(weekday, date, hour, minute)
...
def getRTCdatetime():
	...
	# Year is from 2000
	try:
		return datetime.datetime(year+2000, month, date, hour, minute, second)+argonrtc.getLocaltimeOffset()
	except:
		return datetime.datetime(2000, 1, 1, 0, 0, 0)
...
def setNextAlarm(commandschedulelist, prevdatetime):
	nextcommandtime, weekday, date, hour, minute = argonrtc.getNextAlarm(commandschedulelist, prevdatetime)
	...
	setRTCAlarm(True, nextcommandtime.weekday, nextcommandtime.date, nextcommandtime.hour, nextcommandtime.minute)
	return nextcommandtime
